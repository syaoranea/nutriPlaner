...
@Component({
      ...
      <div class="auth-card glass">
        ...
        <form [formGroup]="form" (ngSubmit)="onSubmit()" class="auth-form">
          ...
          <div class="form-group">
            ...
            <input id="password" [type]="showPassword() ? 'text' : 'password'" formControlName="password" placeholder="Mínimo 6 caracteres" autocomplete="new-password" />
          ...
          <button type="submit" class="btn-submit" [disabled]="loading()">
            @if (loading()) { <span class="spinner"></span> } @else { Criar conta }
          </button>
        </form>
  ...
  styles: [`
    ...
    .auth-bg {
      ...
      inset: 0;
      background:
        radial-gradient(ellipse at 80% 30%, rgba(34,197,94,0.15) 0%, transparent 50%),
...
export class RegisterComponent {
  ...
  private getErrorMessage(code: string): string {
    const messages: Record<string, string> = {
      ...
      'auth/operation-not-allowed': 'O login com E-mail/Senha não está ativado no Firebase Console.',
      'auth/configuration-not-found': 'Projeto Firebase não configurado corretamente. Verifique se o Authentication foi ativado no Console.',
      'auth/popup-closed-by-user': 'Login cancelado.',
    };
    return messages[code] || 'Ocorreu um erro. Verifique o console para mais detalhes.';
  }
}
